...
description: >
  ...
  via render; FormSubmitButton (built on ActionButton) drives loading/success/
  failed from the submit promise; SimpleVForm is the vertical-stack shortcut.
  errorToMessage is required (i18n); FormValidationError distinguishes invalid
  fields from an onSubmit failure. Load when building a form with validation.
type: core
library: alouette
