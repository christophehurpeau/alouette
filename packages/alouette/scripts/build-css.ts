...
//   - src/default-palette.css the default palette (@theme color defaults + the
//                             twelve :where(.<theme>) blocks) = generateTheme().css.
//   - src/global.css          convenience aggregator importing both, so existing
//                             `@import "alouette/global.css"` consumers still work.
//   - src/themeVariables.ts   runtime default map = generateTheme().themeVariables.
...
const { css: defaultPaletteCss, themeVariables } = generateTheme();
...
// --- default-palette.css -----------------------------------------------------
// The default palette: the @theme color defaults (which generate bg-*, text-*,
// border-* utilities) plus the twelve :where(.<theme>) blocks. Optional — import
// after core.css. A BYO app imports its own generateTheme().css instead.
const defaultPaletteFile = `/* Generated by scripts/build-css.ts. DO NOT EDIT.
   Default alouette palette (optional). Import after 'alouette/core.css'. A BYO
   app imports its own generateTheme() palette CSS instead of this file. */
${defaultPaletteCss}`;
...
// --- global.css --------------------------------------------------------------
// Convenience aggregator: structural core + default palette. Keeps existing
// `@import "alouette/global.css"` consumers working. BYO apps import core.css +
// their own palette CSS instead of this file.
const globalCss = `/* Generated by scripts/build-css.ts. DO NOT EDIT.
   Aggregator: alouette's structural core + the default palette. BYO-palette apps
   import "alouette/core.css" + their own generated palette CSS instead. */
@import "./core.css";
...
const defaultThemeVariablesTsPath = join(srcDir, "defaultThemeVariables.ts");
const { code: defaultThemeVariablesTs } = await format(
  defaultThemeVariablesTsPath,
...
writeFileSync(defaultThemeVariablesTsPath, defaultThemeVariablesTs);
console.log(`Wrote ${join(srcDir, "defaultThemeVariables.ts")}`);
